...
-- BEGIN supabase/migrations/202607170001_fix_publication_finalize_idempotency.sql
-- ============================================================================
...
revoke execute on function public.finalize_publication_candidate(uuid,bigint,uuid) from public, anon, authenticated;
grant execute on function public.finalize_publication_candidate(uuid,bigint,uuid) to service_role;
-- END supabase/migrations/202607170001_fix_publication_finalize_idempotency.sql

-- BEGIN supabase/migrations/202607170002_fix_publication_finalize_output_columns.sql
-- Qualify finalize output-column names that collide with table columns.
-- The previous function could fail with ambiguous publication_version or article_id references.
create or replace function public.finalize_publication_candidate(
  p_candidate_id uuid,
  p_expected_publication_version bigint,
  p_actor_id uuid default null
...
begin
  ...
  update public.articles as article
    set privacy_reviewed = case when v_candidate.publication_policy = 'manual_review' then true else article.privacy_reviewed end,
        legal_reviewed = case when v_candidate.publication_policy = 'manual_review' then true else article.legal_reviewed end,
        status = 'published',
        publication_version = p_expected_publication_version + 1,
        updated_at = now()
    where article.id = v_article.id
      and article.publication_version = p_expected_publication_version
    returning article.* into v_article;
  if not found then raise exception 'article publication version conflict' using errcode = '40001'; end if;
  update public.publication_candidates as candidate set state = 'superseded' where candidate.article_id = v_article.id and candidate.state = 'published' and candidate.id <> v_candidate.id;
  update public.publication_candidates set state = 'published', article_id = v_article.id, activated_at = now(), failure_reason = null where id = v_candidate.id;
  update public.article_sources set article_id = v_article.id, state = case when state = 'onboarding' then 'active' else state end, last_error = null where id = v_candidate.source_id;
  update public.article_working_copies as working_copy set article_id = v_article.id where working_copy.id = v_candidate.working_copy_id and working_copy.article_id is null;
  insert into public.content_audit_log (event_type, aggregate_type, aggregate_id, actor_id, payload)
    values ('article.published', 'article', v_article.id, p_actor_id, jsonb_build_object('candidate_id', v_candidate.id, 'publication_version', v_article.publication_version));
  insert into public.content_outbox (topic, aggregate_type, aggregate_id, payload)
    values ('article.published', 'article', v_article.id, jsonb_build_object('article_id', v_article.id, 'candidate_id', v_candidate.id, 'publication_version', v_article.publication_version, 'actor_id', p_actor_id));
...
-- BEGIN supabase/migrations/202607180001_featured_article_management.sql
...
-- END supabase/migrations/202607180001_featured_article_management.sql

-- BEGIN supabase/migrations/202607180002_admin_article_classification.sql
...
notify pgrst, 'reload schema';
-- END supabase/migrations/202607180002_admin_article_classification.sql

-- BEGIN supabase/migrations/202609130001_ignore_article_sources.sql
...
revoke execute on function public.prepare_publication_candidate(uuid,bigint,bigint,uuid) from public, anon, authenticated;
grant execute on function public.prepare_publication_candidate(uuid,bigint,bigint,uuid) to service_role;
-- END supabase/migrations/202609130001_ignore_article_sources.sql
